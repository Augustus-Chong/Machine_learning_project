...
def predict_custom_image(model, image_path, device):
    """Loads a single image, applies robust aspect ratio padding, and predicts."""
    ...
    try:
        ...
        sq_image = Image.new('L', (max_dim, max_dim), 0)
        left = (max_dim - width) // 2
        top = (max_dim - height) // 2
        sq_image.paste(image, (left, top))
        
        image_tensor = transforms.Compose([
            transforms.Resize((28, 28)), 
            transforms.ToTensor(),
        ])(sq_image).unsqueeze(0)
